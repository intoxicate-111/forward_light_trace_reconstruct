...
## v0.8.13: source density × capture integration

This is a separate, controlled **3×3 Full-HD, four-view** experiment on the unchanged scene, without geometry optimization or birth. All sources use v0.8.12's `PER_CHART_PLUS_RING` deterministic phase, AREA_STRATIFIED radii, seed 101, 1,024 fixed charts, fixed 32-step geodesic integration, the existing source mass formulation, and the production `micro=1` packet model. E1/E2/E3 use 32×32 / 48×48 / 64×64 local samples: **1,048,576 / 2,359,296 / 4,194,304 total emitters**. Each chart's unchanged mass is divided by its new local sample count. Geometry, view directions, visibility law, packet radius/shell/step, launch exclusion, ambient .35, gate width .05, sensor gain 1.5 and detector window 2.8 are fixed.

The existing detector is a continuous cubic splat, not an independent camera-ray sampler. C1/C2/C3 therefore use **1×1 / 2×2 / 4×4 deterministic midpoint quadrature of that same cubic response over a unit pixel area**. The offsets are `(j+.5)/q-.5`, with weight `1/q²` per 2D subpixel. C1 is exactly the existing pixel-center cubic operator; C2/C3 increase detector-space integration density without moving emitters, regenerating transport, resizing images or changing the kernel scale. This is a valid detector-integration density control, but not extra independent scene observations. It may converge a slightly smoother pixel-area measurement rather than sharpen it. The unchanged reference is never supersampled or replaced inside the matrix.

Each emitter level has one shared scene/transport state reused across all three capture levels and all detector quadrature samples. The official initializer avoids the old helpers' dense chart-distance tables: fourth-neighbor normal-aware chart spacing is computed one chart at a time, and chart masses come from the verified cache. The spacing formula is unchanged. Four remapped baseline charts agree with the old source positions to `1.89e-15` and normals to `5.85e-14`; E1C1 image replay has relative L2 `6.69e-8`. Existing grid geodesic and finite-packet streaming code is reused, with at most 8,192 geodesic points or detector emitters per chunk. No emitter-by-lambda or all-point-by-chart tensor or dense autograd graph is constructed by this experiment.

All cells report whole/foreground/silhouette MSE, thin and gradient-magnitude proxies, reference vector-gradient error/cosine, unmatched gradient energy, source/energy accounting and runtime/allocated-reserved VRAM/CPU RSS. KD-tree statistics give foreground nearest-center p50/p90/p95/p99 and counts within .5/1/2 pixels, with zero/weak-support fractions. Fixed foreground **pixel-center** statistics are identical across C at fixed E; the separately labeled **subpixel capture-location** statistics use the actual quadrature positions. These counts include all projected source centers, before gate and transmission, and are therefore an optimistic geometric support diagnostic, not a visibility-weighted observation rank. The occupancy sheet integrates unit emitter counts with each detector quadrature, so its brightness increases with source count; RGB source mass remains normalized.

Artifact scores use image-plane angular profiles and radial banding around fixed chart centers, plus unmatched reference-gradient energy. Curvature, overlap and foreshortening can affect these proxies. Matched 160×160 crops include previous spoke and ring locations and the top-z source region; all RGB matrix cells share one display scale. Reference crops remain the original sparse hard-surface splats. Best-cell ranking minimizes reference vector-gradient squared error, using MSE to break ties, not raw thin/HF magnitude. Material improvement requires nonincreased MSE and either at least 1% less gradient error, or at least 20% less unmatched gradient energy together with lower gradient error and higher gradient cosine. These are descriptive thresholds, not confidence tests or proof of recovered geometry bandwidth.

### Density matrix results

Four-view whole-image MSE:

| Emitter density | C1: 1×1 | C2: 2×2 | C3: 4×4 |
|---|---:|---:|---:|
| E1: 32×32/chart | 17.364103 | 17.355013 | 17.353290 |
| E2: 48×48/chart | 17.337016 | 17.334205 | 17.333697 |
| E3: 64×64/chart | 17.329265 | 17.328365 | 17.328206 |

Increasing emitter density reduces unmatched gradient energy by **89.3%** and the radial-banding score by **73.2%**, averaged over C. However, vector-gradient error improves only **0.182%**, and signed gradient cosine worsens by **0.004683**. Increasing capture integration reduces unmatched gradient energy by **25.5%**, but gradient error improves only **0.0340%**, with cosine worsening by **0.000142**. Neither axis passes the declared reference-aligned material-fidelity test: `EMITTER_DENSITY_INCREASE_IMPROVES_FULLHD_FIDELITY=false`, `CAPTURE_DENSITY_INCREASE_IMPROVES_FULLHD_FIDELITY=false`, and **`PRIMARY_LIMITATION=OTHER`**. This does not deny visible denoising; it rejects equating that denoising with recovered geometry fidelity. Cosines are compared by signed differences, never by ratios of potentially negative values; a regression test covers this case.

The selected `BEST_CONFIGURATION=E3C3` minimizes the declared gradient error and MSE, but its thin proxy is **0.001012**, versus E1C1's **0.005510**, and its gradient cosine is **−0.000771**, versus **0.004035**. `THIN_PROXY_STILL_ALIASING_CONTAMINATED=true`. Matched crops show substantially less fine sampling texture as E increases, yet broad chart-centered petals and bright concentric rings remain, sometimes becoming more conspicuous when fine texture disappears. The operational flower flags are true for both E and C because unmatched gradient energy falls by >20%; the ring flag is true for E and false for C. **These flags describe the stated pixel-scale proxies, not disappearance of the visible chart-scale pattern.** Capture integration largely attenuates existing texture; it does not sharpen or generate missing scene detail.

At fixed foreground pixel centers, the four-view mean zero-neighbor fraction within .5 px falls **26.09% → 7.83% → 2.51%** for E1/E2/E3. At E3 the view-mean nearest-center p50/p90/p95/p99 are **0.170/0.352/0.421/0.894 px**. E3 therefore passes the declared <5% zero-neighbor criterion (`FULLHD_SOURCE_OCCUPANCY_STILL_UNDERSAMPLED=false`), not a claim of complete visible support. E3's C2→C3 RGB relative L2 change is **0.004424**, below the 1% numerical-integration threshold (`FULLHD_CAPTURE_SAMPLING_STILL_UNDERSAMPLED=false`). Neither flag establishes continuous-surface or physical-camera convergence.

The remaining explanations are not isolated by these two axes. The frozen 4,096-splat hard reference is not a dense radiance oracle, so reference/readout mismatch remains a major limitation of the fidelity comparison. Hierarchical chart mass and overlap/geodesic-area weighting are unchanged, so denser quadrature cannot remove source-measure bias. The scalar grid and fixed geodesic integration representation still limit geometry bandwidth. The original micro=1 attenuation/visibility model is also fixed, so transport-formulation bias is not ruled out. The matrix supports **density-dependent artifact reduction without Full-HD reference-aligned recovery**, not a unique causal choice among these remaining explanations.

The formal local run takes **393.94 s** including initialization, density diagnostics, all nine four-view cells and figures. E2/E3 mapping takes **48.87/72.11 s** and transport **8.38/12.58 s**; E1 reuses its exact cache. Four-view C1/C2/C3 readout times are **1.07/1.93/5.36 s** at E1, **1.96/4.05/11.96 s** at E2, and **3.25/6.97/21.46 s** at E3. Every readout cell peaks at **457.76 MiB allocated CUDA**; reserved peaks span **548–554 MiB**, and CPU RSS peaks at **3424.45 MiB**. The maximum detector energy discrepancy is **4.17e-7 relative**, and the maximum chart-mass difference is at floating-point roundoff. No HPC or production parameter retuning was used.

Validation passes: compileall, the existing CPU `demo.py --verify`, 21 unit tests, strict JSON/CSV parsing (2,606 CSV entries), all nine Full-HD cell arrays finite, detector energy accounting, nine decoded figures, and `git diff --check`. The validator compares **313 historical artifact/figure files** byte-for-byte with `fa141e3`; none changed. It also verifies per-E cache identity across C, chart-mass consistency, the baseline image replay, initialization equivalence, and the signed-cosine fidelity classification.

Run on the local CUDA machine:

```bash
MPLCONFIGDIR=/tmp/mpl-v0813 PYTHONPATH=src python demo.py --density-matrix
PYTHONPATH=src python -m compileall -q src tests scripts demo.py
MPLCONFIGDIR=/tmp/mpl-v0813 PYTHONPATH=src python demo.py --verify
MPLCONFIGDIR=/tmp/mpl-v0813 PYTHONPATH=src python -m unittest discover -s tests -v
MPLCONFIGDIR=/tmp/mpl-v0813 PYTHONPATH=src python scripts/validate_v0813.py
git diff --check
```
